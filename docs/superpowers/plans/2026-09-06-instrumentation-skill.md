...
5. **`adb logcat -G 16M` (main spec §5.2) was deliberately left out of this branch, and has since been implemented on its own.** *(Closed: `src/adb/logcat-buffer.ts`, called from `state attach` and from `launch` when it attaches.)* The original reasoning: Raising the logcat ring is a behaviour change to a phase-3 component — the capture — and it deserves its own test cycle rather than riding in on an `init`/skill/doctor branch. Its absence degrades loudly rather than silently: a smaller ring means more dropped lines, and dropped lines are exactly what sequence-gap detection already reports as staleness. Recorded here so it is not lost.
6. **A `|`, newline or other control character in a state key is sanitised in the emitter, not merely documented.** `key` is interpolated straight into the `|`-delimited wire line, so such a key produced a line `parseWireLine` rejects and the record vanished with no trace on either side. `AgentQa.emit` now replaces those characters with `_`, which keeps the record visible in `agentqa state list` — a mangled key that matches nothing is discoverable; a missing record is not. The skill states the constraint as well.
7. **`skill version` compares ordinally, where the spec says only "older".** Inequality either way meant a repo stamped by a *newer* agentqa was told to re-run `init`, which would downgrade it, and any patch release made `doctor` exit 1 in every initialised repo on the machine. Older by more than a patch is `fail`; newer, patch-only, or incomparable is `unknown`.
